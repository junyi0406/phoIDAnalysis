...
def df_pteta_rwt(df, sigvar, backvar,
    ...
    sig_wei, xedge, yedge = np.histogram2d(sig_pt, sigl_eta, bins=[ptbins,etabins_barrel], density=False)
    back_wei, xedge, yedge = np.histogram2d(back_pt, back_eta, bins=[ptbins,etabins_barrel], density=False)
    # do normalization
    ...
    for i in range(len(ptbins)-1):
        for j in range(len(etabins_barrel)-1):
            df.loc[ (df["Category"] == sigvar)
                    ...
                    & (df[etavar]>etabins_barrel[j]), NewWeightCol] = weight[i, j]
            df.loc[ (df["Category"] == backvar)
                    & (df["region"]=="barrel") 
                    & (df[ptvar]<ptbins[i+1]) 
                    & (df[ptvar]>ptbins[i])
                    & (df[etavar]<etabins_barrel[j+1])
                    & (df[etavar]>etabins_barrel[j]), NewWeightCol] = 1.0
    ...
    sig_wei, xedge, yedge = np.histogram2d(sig_pt, sigl_eta, bins=[ptbins,etabins_endcap], density=False)
    back_wei, xedge, yedge = np.histogram2d(back_pt, back_eta, bins=[ptbins,etabins_endcap], density=False)
    # remove barrel val
    ...
    for i in range(len(ptbins)-1):
        for j in range(len(etabins_endcap)-1):
            df.loc[ (df["Category"] == sigvar)
                        ...
                        & (df[etavar]>etabins_endcap[j]), NewWeightCol] = weight[i, j]
            df.loc[( df["Category"] == backvar)
                        & (df["region"]=="endcap") 
                        & (df[ptvar]<ptbins[i+1]) 
                        & (df[ptvar]>ptbins[i])
                        & (df[etavar]<etabins_endcap[j+1])
                        & (df[etavar]>etabins_endcap[j]), NewWeightCol] = 1.0
